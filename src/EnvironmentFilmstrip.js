...
function EnvironmentFilmstrip({
  ...
  const systemEnvs = filteredEnvironments.filter(
    (env) => !(env.is_user_env ?? env.isUserEnv)
  );
  const userEnvs = filteredEnvironments.filter(
    (env) => env.is_user_env ?? env.isUserEnv
  );
  ...
  const renderEnvTile = (env) => {
    ...
    const isUserEnv = env.is_user_env ?? env.isUserEnv;
